...
async function launchPlatform() {
    try {
        await fin.System.launchManifest("https://built-on-openfin.github.io/container-starter/dev/nick/domain-permissions/use-channel-api-issue-commands-to-a-platform/platform.fin.json");
    }
    catch (error) {
...
async function requestView() {
    try {
        ...
        lastCreatedView = await channelClient.dispatch("createView", {
            viewOptions: {
                url: "https://built-on-openfin.github.io/container-starter/dev/nick/domain-permissions/use-channel-api-issue-commands-to-a-platform/html/app.html",
                customData: {
                    dateNow: Date.now()
                }
            }
