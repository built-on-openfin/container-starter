 ...
 */
export async function connectToExternalClient(
 ...
 */
export async function validateBrokerConnection(
	externalClient: ExternalClient,
	payload: { HERE_API?: string }
): Promise<boolean> {
	const platform: OpenFin.Platform = fin.Platform.wrapSync({ uuid: externalClient.uuid });
	const isValidManifest = await validateManifestUrl(externalClient, platform);
	if (!isValidManifest) {
		console.error(
			`Validation failed for platform with uuid ${externalClient.uuid} due to manifest url mismatch.`
		);
		return false;
	}
	...
	if (!isValidPayload) {
		console.error(
			`Validation failed for platform with uuid ${externalClient.uuid} due to invalid custom data. Received HERE_API: ${payload.HERE_API}`
		);
		return false;
	}
	return true;
 ...
 */
async function validateAndReturnBrokerConnection(
	externalClient: ExternalClient,
	platform: OpenFin.Platform
): Promise<OpenFin.InteropClient> {
 ...
 * @returns A boolean indicating if the connection is valid.
 */
async function validateManifestUrl(
	externalClient: ExternalClient,
	platform: OpenFin.Platform
): Promise<boolean> {
	...
	if (!isValid) {
		console.warn(
			`Validating connection to platform with uuid ${externalClient.uuid}. Expected manifest url: ${externalClient.manifestUrl}, Actual manifest url: ${info.manifestUrl}`
		);
	}
	return isValid;
}
...
async function validatePayload(
	...
	if (!isPayloadValid) {
		...
		console.warn(
			`Validating connection to platform with uuid ${externalClient.uuid}. Expected HERE_API: ${externalClient.interopKey}, Actual HERE_API: ${payload.HERE_API}`
		);
	}
