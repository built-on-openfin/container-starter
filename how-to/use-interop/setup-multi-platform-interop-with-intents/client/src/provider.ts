...
function interopOverride(InteropBroker: OpenFin.Constructor<OpenFin.InteropBroker>): OpenFin.InteropBroker {
	...
	class Override extends InteropBroker {
		...
		public async handleInfoForIntent(
			...
			console.log(
				"Received request for info for intent:",
				intentOptions,
			...
			if (response.apps.length === 0) {
				...
				console.info(
					`No external clients support intent ${intentOptions.name}. Returning this platform as the only option to handle the intent but it may not be able to handle it if it doesn't have a listener for it.`
				);
			}
		...
		public async handleFiredIntent(
			...
			if (intent.metadata?.target?.appId && intent.metadata?.target?.appId !== fin.me.identity.uuid) {
				console.log(
					`Intent ${intentName} has a target app ${intent.metadata.target.appId}. Attempting to find a matching external client for it.`
				);
				targetExternalClient = this.externalClients.find(
					(externalClient) =>
						externalClient.uuid === intent.metadata?.target?.appId &&
						externalClient.interop.intents?.listensFor &&
				...
				if (targetExternalClient) {
					if (targetExternalClient.appId && targetExternalClient.appId !== intent.metadata?.target?.appId) {
						console.warn(
							`The intent ${intentName} has a target appId ${intent.metadata.target.appId} that matches the uuid of an external client but it does not match the preferred appId for that external client. Updating it to ${targetExternalClient.appId}`
						);
					...
					console.log(
						`Found matching external client ${targetExternalClient.uuid} for intent ${intentName} with target appId ${intent.metadata?.target?.appId}. Routing intent to it.`
					);
				} else {
			...
			if (targetExternalClient) {
				console.log(
					`Found matching external client ${targetExternalClient.uuid} for intent ${intentName}. Routing intent to it.`
				);
				const targetConnection = this.externalClientConnections.find(
					(connection) => connection.uuid === targetExternalClient.uuid && connection.client
				);
				...
				console.warn(
					`External interop client connection for ${targetExternalClient.uuid} was not found. Unable to fire intent.`
				);
				throw new Error("TargetAppUnavailable");
			}
			// For this example we are going to launch a url that matches the intent name and pass it the context.
			const url =
				"https://built-on-openfin.github.io/dev-extensions/main/interop/fdc3/intent/2-0/fdc3-intent-view.html";
			const targetIdentity = {
				uuid: fin.me.identity.uuid,
				name: `${intentName.toLocaleLowerCase()}-${randomUUID()}`
			};
		...
		public async isActionAuthorized(
			...
			if (
				...
				if (!isAllowedChannel) {
					console.warn(
						`Connection request from ${JSON.stringify(identity)} attempted to join context group ${payload.contextGroupId} which is not in the allowed channels list and was rejected.`
					);
					return false;
				}
				return true;
			...
			if (
				action === "intentHandlerRegistered" &&
				payload &&
				typeof payload === "object" &&
				...
				if (!allowedIntentListeners.includes(requestedIntent)) {
					console.warn(
						`Connection request from ${JSON.stringify(identity)} attempted to register an intent handler for ${requestedIntent} which is not in the allowed intents list and was rejected.`
					);
					return false;
				}
				return true;
			...
			if (
				...
				if (!allowedIntentsToRaise.includes(requestedIntent)) {
					console.warn(
						`Connection request from ${JSON.stringify(identity)} attempted to fire an intent ${requestedIntent} which is not in the allowed intents list and was rejected.`
					);
					return false;
				}
				return true;
		...
		public async initializeConnections(): Promise<void> {
			...
			const externalClientEndpointUrl = location.href.replace(
				"provider.html",
				`${fin.me.identity.uuid.toLocaleLowerCase().replaceAll(" ", "-")}-external-clients.json`
			);
			...
			try {
				...
				if (response.ok) {
					...
					console.warn(
						`Failed to fetch external clients from ${externalClientEndpointUrl}. Using default external clients. Response status: ${response.status}`
					);
				}
			} catch (error) {
				console.error(
					`Error fetching external clients from ${externalClientEndpointUrl}. Using default external clients. Error:`,
					error
				);
			}
			for (const externalClient of fetchedExternalClients) {
				...
				this.validateAndConnect(externalClient).catch((error) =>
					console.error(`Error connecting to external broker ${externalClient.uuid}:`, error)
				);
			}
		}
		...
		private async validateAndConnect(externalClient: ExternalClient): Promise<void> {
			...
			await connection.client.onDisconnection((event) => {
				...
				this.externalClientConnections = this.externalClientConnections.filter(
