...
async function run(manifestUrl) {
	try {
		...
		// #region agent log
		fetch('http://127.0.0.1:7242/ingest/1fe79a6f-d76b-42bb-97bd-71d0a412af2b', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				location: 'launch.mjs:run',
				message: 'Connection/launch error',
				data: { message: String(e?.message), stack: String(e?.stack) },
				timestamp: Date.now(),
				hypothesisId: 'launch-error'
			})
		}).catch(() => {});
		// #endregion
	}
...
async function launchFromNodeAdapter(manifestUrl) {
	...
	if (fin) {
		...
		console.log();
		console.log(`Close all platform windows to close the platform.`);
...
async function connectAndGetFinAPI(manifestUrl, exitMethod) {
	try {
		...
		fetch('http://127.0.0.1:7242/ingest/1fe79a6f-d76b-42bb-97bd-71d0a412af2b', {
			...
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				location: 'launch.mjs:connectAndGetFinAPI',
				message: 'Manifest launch failed',
				data: { message: String(e?.message), stack: String(e?.stack) },
				timestamp: Date.now(),
				hypothesisId: 'manifest-launch'
			})
		}).catch(() => {});
		// #endregion
	}
